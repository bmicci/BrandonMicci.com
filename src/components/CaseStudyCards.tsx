...
export default function CaseStudyCards() {
  const caseStudies = [
    ...
    },
    {
      ...
      description:
        'Modernized legacy systems across multiple industries while building adoption communities.',
    ...
    },
    {
      ...
      description:
        'Built transformative solutions spanning financial services, aviation, and consulting.',
    ...
    },
    {
      ...
      description:
        'Translated complex AI strategies into clear business roadmaps and competitive advantages.',
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {caseStudies.map((study, index) => (
        <article
          key={index}
          className="rounded-xl ring-1 ring-white/10 p-6 bg-white/5 hover:bg-white/10 transition-colors"
        >
          <div className="text-2xl mb-3">{study.icon}</div>
          <h3 className="text-xl font-semibold mb-2">{study.title}</h3>
          <p className="opacity-80 mb-4">{study.description}</p>
          <div className="text-sm opacity-80 font-medium text-brand-cyan">
            {study.outcome}
          </div>
        </article>
      ))}
